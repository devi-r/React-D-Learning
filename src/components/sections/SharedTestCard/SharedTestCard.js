...
const SharedTestCard = ({ test }) => {
  const subjectName = subjectCodeMap[test.subject] || "General";
  const subjectStyles = getSubjectCSSVars(subjectName);

  return (
    <div className="shared-test-card" style={subjectStyles}>
      <div className="shared-test-card__content">
        <SubjectIcon subjectCode={test.subject} />
        <div className="shared-test-card__details">
          <p className="shared-test-card__title">{test.title}</p>
          <p className="shared-test-card__duration">{test.duration}</p>
        </div>
      </div>
      <MdArrowForward className="shared-test-card__add-icon" size={20} />
    </div>
  );
};
